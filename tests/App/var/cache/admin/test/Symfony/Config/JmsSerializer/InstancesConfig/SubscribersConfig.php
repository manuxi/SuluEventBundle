...
class SubscribersConfig 
{
    ...
    {
        if (array_key_exists('doctrine_proxy', $config)) {
            $this->_usedProperties['doctrineProxy'] = true;
            $this->doctrineProxy = new \Symfony\Config\JmsSerializer\InstancesConfig\Subscribers\DoctrineProxyConfig($config['doctrine_proxy']);
            unset($config['doctrine_proxy']);
        }
