...
class DefaultContextConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('serialization', $config)) {
            $this->_usedProperties['serialization'] = true;
            $this->serialization = \is_array($config['serialization']) ? new \Symfony\Config\JmsSerializer\DefaultContext\SerializationConfig($config['serialization']) : $config['serialization'];
            unset($config['serialization']);
        }

        if (array_key_exists('deserialization', $config)) {
            $this->_usedProperties['deserialization'] = true;
            $this->deserialization = \is_array($config['deserialization']) ? new \Symfony\Config\JmsSerializer\DefaultContext\DeserializationConfig($config['deserialization']) : $config['deserialization'];
