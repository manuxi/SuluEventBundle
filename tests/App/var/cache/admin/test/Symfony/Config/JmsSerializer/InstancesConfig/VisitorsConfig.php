...
class VisitorsConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('json_serialization', $config)) {
            $this->_usedProperties['jsonSerialization'] = true;
            $this->jsonSerialization = new \Symfony\Config\JmsSerializer\InstancesConfig\Visitors\JsonSerializationConfig($config['json_serialization']);
            unset($config['json_serialization']);
        }

        if (array_key_exists('json_deserialization', $config)) {
            $this->_usedProperties['jsonDeserialization'] = true;
            $this->jsonDeserialization = new \Symfony\Config\JmsSerializer\InstancesConfig\Visitors\JsonDeserializationConfig($config['json_deserialization']);
            unset($config['json_deserialization']);
        }

        if (array_key_exists('xml_serialization', $config)) {
            $this->_usedProperties['xmlSerialization'] = true;
            $this->xmlSerialization = new \Symfony\Config\JmsSerializer\InstancesConfig\Visitors\XmlSerializationConfig($config['xml_serialization']);
            unset($config['xml_serialization']);
        }

        if (array_key_exists('xml_deserialization', $config)) {
            $this->_usedProperties['xmlDeserialization'] = true;
            $this->xmlDeserialization = new \Symfony\Config\JmsSerializer\InstancesConfig\Visitors\XmlDeserializationConfig($config['xml_deserialization']);
