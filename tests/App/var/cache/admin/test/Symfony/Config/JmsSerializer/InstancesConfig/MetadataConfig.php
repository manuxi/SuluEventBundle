...
class MetadataConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('warmup', $config)) {
            $this->_usedProperties['warmup'] = true;
            $this->warmup = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\WarmupConfig($config['warmup']);
            unset($config['warmup']);
        }
        ...
        if (array_key_exists('file_cache', $config)) {
            $this->_usedProperties['fileCache'] = true;
            $this->fileCache = new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\FileCacheConfig($config['file_cache']);
        ...
        if (array_key_exists('directories', $config)) {
            $this->_usedProperties['directories'] = true;
            $this->directories = array_map(fn ($v) => new \Symfony\Config\JmsSerializer\InstancesConfig\Metadata\DirectoryConfig($v), $config['directories']);
